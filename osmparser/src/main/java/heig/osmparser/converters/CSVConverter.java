...
public class CSVConverter extends MainControllerHandler {
    ...
    public void rawGraphToCSV(Graph g) {
        try {
            ...
            edgesWriter.write("from_node_id,to_node_id,time_cost,length_cost\n");
            for(long i : g.getAdjList().keySet()) {
                ...
                for (Way w: g.getAdjList().get(i)) {
                    long n2Id = w.getToId();
                    edgesWriter.write(n1.getId() + "," + n2Id + "," + w.getTimeCost() + "," + w.getLengthCost() + "\n");
                }
            }
    ...
    public void flattenGraphToCSV(Graph g) {
        try {
            ...
            edgesWriter.write("from_node_id,to_node_id,from_city_name,to_city_name,time_cost,length_cost\n");
            ...
            for(int i = 0; i < citiesIds.size(); ++i) {
                ...
                for (int j = 0; j < citiesIds.size(); ++j) {
                    ...
                    double c_ij = g.getLambdaTime().get(n2.getId());
                    double c_ij_p = g.getLambdaLength().get(n2.getId());
                    edgesWriter.write(city1.getId() + "," + city2.getId() + ","
                            + city1.getName() + "," + city2.getName() + ","
                            + Maths.round(c_ij, 4) * Config.SPEED_SMOOTH_FACTOR + ","
                            + Maths.round(c_ij_p, 4) + "\n");
                }
            }
            sendMessageToController("export done", Log.LogLevels.INFO);
            nodesWriter.close(); edgesWriter.close();
